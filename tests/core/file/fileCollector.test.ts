...
describe('fileCollector', () => {
  ...
  it('should skip binary files', async () => {
    ...
    expect(logger.debug).toHaveBeenCalledWith('Skipping binary file: /root/binary.bin');
  });
  ...
  it('should handle file read errors', async () => {
    ...
    expect(result).toEqual([]);
    expect(logger.warn).toHaveBeenCalledWith('Failed to read file: /root/error.txt', expect.any(Error));
  });
});
